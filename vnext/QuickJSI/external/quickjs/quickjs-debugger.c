...
#pragma warning(disable:4311) //Disable 'type cast': pointer truncation
#endif
...
static void js_normalize_path(const char* path, char buffer[], int buff_len) {
    const char* pathPtr = path;
    char* buffPtr = buffer;
    while (*pathPtr != 0 && buffPtr - buffer < buff_len) {
        if (*pathPtr == '\\') {
            *buffPtr++ = '/';
            pathPtr++;
        }
...
static void js_process_breakpoints(JSDebuggerInfo *info, JSValue message) {
    ...
    js_normalize_path(path, normalizedPath, 260);

    JSValue path_data = JS_GetPropertyStr(ctx, info->breakpoints, normalizedPath);
    ...
    JS_SetPropertyStr(ctx, info->breakpoints, normalizedPath, path_data);
